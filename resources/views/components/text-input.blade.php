<div class="relative">
    @if ('textarea' !== $type)
        @if ($formRef)
            <button type="button"
                class="absolute top-0 right-0 flex h-full items-center px-2 text-slate-400 hover:text-slate-600"
        ...
        <input x-ref="input-{{ $name }}" type="{{ $type }}" placeholder="{{ $placeholder }}"
            ...
            @class([
                'w-full rounded-md border-0 px-2.5 py-1.5 text-sm ring-1 placeholder:text-slate-400 focus:ring-2 ',
                'pr-8' => $formRef,
